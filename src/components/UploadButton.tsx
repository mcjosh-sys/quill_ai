...
const UploadDropzone = ({isSubscribed}:{isSubscribed: boolean}) => {
    ...
    return <Dropzone multiple={false} onDrop={async (acceptedFile) => {
        ...
        !res && toast({ title: "Something went wrong", description: "Please try again later", variant: "destructive" })

        const fileResponse = res?.[0]

        const key = fileResponse?.key || ''

        !key && toast({ title: "Something went wrong", description: "Please try again later", variant: "destructive" })
